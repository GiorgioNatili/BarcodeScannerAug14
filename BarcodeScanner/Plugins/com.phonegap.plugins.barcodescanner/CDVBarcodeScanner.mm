...
@property (nonatomic)         CGFloat                     updatedScaleDelta;
...
- (void)barcodeScanCancelled {
    [self barcodeScanDone];
    [self.plugin returnSuccess:@"not_valid" format:@"" cancelled:TRUE callback:self.callback];
}
...
- (void)captureOutput:(AVCaptureOutput*)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection*)connection {
    ...
    effectiveScale = effectiveScale * self.updatedScaleDelta;
    ...
    // TODO populate the field only if the zoom is active
    NSString* aString = [NSString stringWithFormat:@"%.1f%%", (effectiveScale * 100)];
    self.viewController.zoomLabel.text = aString;
...
- (void)loadView {
    ...
    // Try to add some gesture handling here
    
    /*  UIPinchGestureRecognizer *pinchRecognizer = [[[UIPinchGestureRecognizer alloc] initWithTarget:self.view action:@selector(scale:)] autorelease];
...
- (void)twoFingerPinch:(UIPinchGestureRecognizer *)recognizer
{
    ...
    // Zooming is applied just when the user end the gesture
    if([recognizer state] == UIGestureRecognizerStateBegan){
        
        self.processor.zooming = true;
        
    }
    
    if([recognizer state] == UIGestureRecognizerStateEnded){
        
        self.processor.zooming = false;
        
    }
    
    // This is the scale applied to thesture recognizer
    if(recognizer.scale > 0.3 && recognizer.scale < 4.0){
        
        // It should be between 0.3 and 4.0 in order to avoid processor intensive operations
        self.processor.updatedScaleDelta = recognizer.scale;
...
- (UIView*)buildOverlayView {
    ...
    int zoomLabelHeight = 42;
    float zoomLabelX    = rectArea.origin.x + (rectArea.size.width - zoomLabelWidth)/2;
    float zoomLabelY    = rectArea.origin.y - zoomLabelHeight;//(rectArea.size.height);
    
    self.zoomLabel = [[UILabel alloc]initWithFrame:CGRectMake(zoomLabelX, zoomLabelY, zoomLabelWidth, zoomLabelHeight)];
    [self.zoomLabel setBackgroundColor:[UIColor clearColor]];
    [self.zoomLabel setTextAlignment:(NSTextAlignmentCenter)];
    [self.zoomLabel setFont:([UIFont fontWithName:@"HelveticaNeue-Bold" size:32])];
    [self.zoomLabel setTextColor:([UIColor grayColor])];
    [self.zoomLabel setText:@"100%"];
    [[self view] addSubview:self.zoomLabel];
    [self.zoomLabel release];
    
    
    return overlayView;
}

-(void)addRecticleTo:(UIView*)view:(CGRect)rectArea{
